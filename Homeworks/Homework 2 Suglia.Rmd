...
One way to assess model performance is with cross-validation.
...
Use data on 1st fire to predict 2nd fire
...
```{r, message = FALSE}
mskate <- lmer(score~pair + judge + (1|judge) + (1|pair), data = d2)
```
